...
async function handler(
  ...
  try {
    ...
    const { max_size, sats_fee, base_fee } = data1;

    if (amt > max_size) {
      return res
        .status(200)
        .json({ ok: false, err: "Amount exceeds max channel size!" });
    }

    const charge = (Math.floor(amt * sats_fee) + base_fee) * 1000;
    ...
    if (payment_request === undefined || hash === undefined) {
      return res
        .status(200)
        .json({ ok: false, err: "Error fetching invoice from server!" });
    }
